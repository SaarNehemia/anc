...
fs = 44100
sig = v
psd_sig = anc.calc_psd(sig=sig, fs=fs)
psd_d = anc.calc_psd(sig=d, fs=fs)
psd_u = anc.calc_psd(sig=u, fs=fs)
psd_y = anc.calc_psd(sig=e, fs=fs)
anc.plot_psds(psds=[psd_sig, psd_d, psd_u, psd_y],
          titles=['Original (sig)', 'Speech sensor (d)', 'Noise sensor (u)', 'Result (y)'])
